...
| 项目                             | 你的填写 |
| -------------------------------- | -------- |
| 修改后的 `AllowedIPs`            | client：10.30.0.0/24         |
| `latest handshake` 是否还在      | 是，握手时间正常存在         |
| 去 `10.20.0.0/24` 的路由是否消失 |  是，无该路由条目，新增10.30.0.0/24 dev wg0        |
| 是否还能访问 `10.20.0.2:8080`    |    否，连接超时 Connection timed out      |
| 失败的直接原因                   | client 无 10.20.0.0/24→wg0 路由，报文不走 VPN 隧道，直接丢弃 / 走默认路由         |

> 简答：为什么 `wg show` 显示隧道还在，但业务访问已经失败？隧道握手和业务数据传输用的是两套不同的路径还是同一套？
外层 underlay = 信封（只记录收发人外层地址、UDP 封装），内层 wg0 = 信纸（原始通信 IP + 明文数据）；VPN 封装实现传输内容加密 + 通信地址隐藏。

截图：

![alt text](fail_route.png)
...
| ------------------------ | ---------------------------------- | -------- |
| 看到的源地址             | underlay 接口 (veth-vpn-ul)                                   | 10.10.10.1（隧道内层 IP）         |
| 看到的目的地址           | 192.0.2.1（vpn 底层公网 IP）                                   | 10.20.0.2（server 内网 IP）         |
| 协议                     | UDP（51820 端口）                                   |ICMP/TCP（原始报文协议）          |
| 是否能看到 ICMP/HTTP 内容 | 否，密文十六进制，无应用明文                                   | 是，完整 ICMP echo、HTTP GET 明文         |
| 为什么内外两层看到的不同 | WireGuard 封装：原始 IP 包整体加密封装进外层 UDP 报文；wg0 解密剥离外层 UDP，还原原始内层 IP 报文                                   |    未见      |
...
1. WireGuard 的 `AllowedIPs` 做了哪两件不同的事？为什么它"看起来像访问控制，实际上首先影响的是路由"？
   ...
   > 答：出口路由（发包方向）：wg-quick自动将 AllowedIPs 网段写入系统路由表，目标 IP 匹配网段的报文路由至 wg0 虚拟网卡，进入 VPN 隧道加密封装发送。
入口白名单（收包方向）：对端解密后的内层报文，源 IP 必须落在本端 Peer 的 AllowedIPs 网段内，否则直接丢弃报文，实现入站源 IP 访问控制。
优先影响路由原因：wg-quick 启动时先写入操作系统路由，路由决定报文是否进入隧道；访问控制是报文解密后的二次校验。路由失效则报文根本不进隧道，因此路由优先级高于 ACL 过滤，看起来像路由配置而非单纯防火墙规则。

2. 本实验中，`wg show` 显示隧道在线，但 `curl` 失败。为什么隧道握手和业务数据传输可以互相独立？

   > 答：握手（隧道保活）：使用Endpoint配置的底层 underlay 公网 IP（192.0.2.x）+UDP51820，仅校验公私钥合法性，和 AllowedIPs 配置无关，只要底层 UDP 可达即可完成握手、保活，wg show 显示在线。
业务数据：依赖 AllowedIPs 控制路由转发 + 入站源 IP 白名单，路由缺失报文无法进隧道、入站源不匹配报文被丢弃。
二者走两套寻址逻辑（外层 underlay 握手、内层隧道 IP 传数据），因此握手正常≠业务可达。

3. 如果只建立隧道而不限制 AllowedIPs（写成 `0.0.0.0/0`），会带来什么风险？

   > 答：全流量隧道：本机所有 IP 流量全部路由进 VPN 隧道，本机上网全走 VPN 网关，带宽占用高（全隧）。
安全风险：入站白名单失效，对端 Peer 可以伪造任意源 IP 报文进入内网，内网服务器暴露在异常源 IP 攻击中；VPN 被攻陷后攻击者可借助隧道横向渗透内网全段。
路由冲突：默认路由指向 wg0，内网 / 公网路由错乱，网络可用性下降。

4. WireGuard 的公私钥模型和 SSH 很像。假设攻击者只拿到了 vpn 节点的私钥，但没有 client 节点的私钥，他能以 client 的身份成功建立隧道吗？为什么？

   > 答：不能。WireGuard 身份认证：客户端使用自身私钥签名报文，VPN 网关使用客户端公钥（写在 vpn 配置 Peer 段）验签；攻击者无 client 私钥，无法生成合法签名报文，VPN 网关通过 client 公钥校验失败直接丢弃握手包，无法冒充客户端建立隧道。vpn 私钥仅用于解密收到的报文、签发本端出站报文，不能伪造客户端身份。

5. 底层抓包看到的外层源地址是什么？为什么抓包者看不到内层的真实通信内容？

   > 答：外层源地址：client 底层 underlay 192.0.2.2，目的192.0.2.1，UDP51820。
内层不可见原因：WireGuard 使用对称加密算法对整个原始 IP 报文全加密，原始 IP 头、ICMP/HTTP 应用数据全部变成密文，被封装在外层 UDP 载荷中，底层抓包只能看到加密后的二进制密文，无法解析内层地址与明文数据。

6. 任务四中内外两层抓包看到的信息完全不同，这说明 VPN 的"封装"机制给通信提供了哪两方面的安全保障？

   > 答：内容机密性：内层原始报文全加密，中间链路抓包无法窃取应用明文（HTTP/ICMP 数据）。
通信身份隐匿：外层仅暴露底层公网 IP，隐藏两端真实内网 IP（10 段隧道 IP），中间人无法获知真实通信两端内网地址，规避 IP 溯源与定向攻击。
