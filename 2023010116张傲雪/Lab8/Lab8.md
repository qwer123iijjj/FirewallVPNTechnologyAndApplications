...
| `wan` 无法回复的原因 |WAN 删除了去往 10.0.0.0/24 的回程路由，无法将 ICMP 应答包路由回内网 LAN；公网没有私有网段路由，无法回包 |
...
答：额外记录了内网真实地址与 NAT 转换后公网地址的映射关系（内网源 IP、转换后源 IP、目的 IP、协议标识等）。
说明连接跟踪（conntrack）是状态防火墙与 NAT 的共同底层基础，二者都需要维护连接状态信息，实现流量识别与地址转换。

2. 如果两台内网主机同时访问同一个外网服务器的同一个端口，NAT 如何区分两条连接的回复分别属于哪台内网主机？
答：NAT 使用不同的源端口号区分：两台内网主机发出的数据包，NAT 后分配不同的临时端口；wan 回复包目的端口不同，防火墙通过 conntrack 里的端口映射，把流量转发给对应内网主机。
...
| TCP 38070 访问是否成功 |失败 |
| ping 是否成功 |失败 |
| ping 失败的原因 |FORWARD 链默认丢弃所有流量，仅放行 TCP 38070 的新连接，未放行 ICMP 协议，ping 被防火墙拦截 |
...
1. 为什么私有地址（如 `10.0.0.0/8`）的主机通常无法被互联网直接访问？NAT 是怎么解决这个问题的？
   ...
   > 答：因为私有地址仅在内网有效，公网路由表没有私有网段的路由，全球大量内网复用同一批私有地址，公网无法定位具体内网主机。NAT 将内网私有地址统一转换为路由器公网地址，对外隐藏内网结构，公网只需与路由器通信，路由器再通过 conntrack 将流量转发给对应内网主机。

2. `MASQUERADE` 和 `SNAT --to-source 192.0.2.1` 在本实验中效果相同。实际场景中应如何选择？

   > 答：出口 IP固定（如静态公网 IP）：用 SNAT，性能更好，出口 IP动态变化（PPPoE 拨号、DHCP 获取）：用 MASQUERADE，自动跟随接口 IP，无需手动改规则。

3. NAT 对外隐藏了内网拓扑，但它给哪些应用层协议带来了麻烦？为什么？

   > 答： FTP、SIP、游戏联机、P2P 等协议。因为这类协议数据包内部会携带内网 IP / 端口，NAT 只改写 IP 头，不改写应用层载荷，导致对端无法建立连接。

4. 状态检测和 NAT 都依赖 conntrack，从这个角度看，为什么家用路由器通常把这两个功能集成在一起？

   > 答：conntrack 可同时提供连接状态识别和地址端口映射，共用一套连接表，实现防火墙安全控制 + NAT 地址转换，硬件成本低、效率高，适合家用场景。

5. 任务五里 ping 被 FORWARD 链拦截。如果要在保留现有规则的基础上，让 ICMP 也能通过，应该怎么写规则？

   > 答：sudo ip netns exec fw iptables -A FORWARD -p icmp -j ACCEPT
