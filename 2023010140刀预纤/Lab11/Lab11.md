...
| 项目                                   | 你的填写 |
| -------------------------------------- | -------- |
| 是否看到 `latest handshake`            |  是，存在握手时间记录 |
| 是否看到 `transfer` 计数               |  是，存在收发字节统计|
...
| 是否能访问 `10.20.0.2:8080`  |  可以正常访问，返回网页目录 HTML 内容        |
| `ip route` 中 wg0 相关的条目 |   10.10.10.0/24 dev wg0 proto kernel scope link src 10.10.10.1、10.20.0.0/24 dev wg0 scope link、192.0.2.0/24 dev veth-client proto kernel scope link src 192.0.2.2     |
...
| `latest handshake` 是否还在      |   存在，16 秒前完成握手       |
| 去 `10.20.0.0/24` 的路由是否消失 |   已消失，新增10.30.0.0/24 dev wg0路由条目       |
| 是否还能访问 `10.20.0.2:8080`    |  无法访问，curl 连接超时失败        |
| 失败的直接原因                   |  系统无去往10.20.0.0/24指向 wg0 的路由，数据包不会送入 WireGuard 隧道转发       |

> 简答：为什么 `wg show` 显示隧道还在，但业务访问已经失败？隧道握手和业务数据传输用的是两套不同的路径还是同一套？
>答： 隧道握手与业务数据两套独立路径。握手依靠外层 underlay 的 UDP（192.0.2.x）通信，不受 AllowedIPs 影响；业务转发由 AllowedIPs 生成的系统路由控制，路由异常就无法入隧道，因此隧道握手在线但业务不通。
...
| 协议                     |       UDP                             |    ICMP      |
| 是否能看到 ICMP/HTTP 内容 | 不能，全是加密密文，无法解析业务内容  | 可以，明文展示 ICMP 请求、应答数据 |
| 为什么内外两层看到的不同 | WireGuard 将原始内网 IP 报文整体加密，再封装全新 UDP 头部在底层传输；解密前外层只能看到封装后的 UDP 报文 | 报文经过 wg 接口解密，还原出原始 ICMP 内网报文，因此地址、协议、载荷和外层完全不一样 |

> 简答：如果把 VPN 隧道比喻成"信封+信纸"，outer（underlay）抓到的信息对应什么？inner（wg0）抓到的信息对应什么？这样设计对安全性意味着什么？
> 答：outer（underlay）对应信封，只记录两端公网地址；inner（wg0）对应信纸，存放真实内网通信数据。
安全性：公网链路窃听者只能获取加密信封，看不到内部真实 IP 与通信明文，实现数据加密传输、隐藏内网拓扑，防止中间人窃听与窥探内网信息。
...
1. WireGuard 的 `AllowedIPs` 做了哪两件不同的事？为什么它"看起来像访问控制，实际上首先影响的是路由"？
   ...
   > 答：AllowedIPs 两件作用：①自动在本机生成对应网段路由，匹配流量送入 wg 隧道；②对入站报文做访问控制，只放行源 IP 匹配规则的报文。
系统优先生成内核路由，数据包根据路由决定是否进隧道，因此优先影响路由，再实现访问控制效果。

2. 本实验中，`wg show` 显示隧道在线，但 `curl` 失败。为什么隧道握手和业务数据传输可以互相独立？

   > 答：握手基于外层 UDP（192.0.2.x）完成密钥协商，不受 AllowedIPs 路由规则影响；业务转发依赖 AllowedIPs 生成的系统路由。路由异常时握手报文仍可达，业务流量无对应路由无法送入隧道，二者底层逻辑独立。

3. 如果只建立隧道而不限制 AllowedIPs（写成 `0.0.0.0/0`），会带来什么风险？

   > 答：0.0.0.0/0会生成默认路由，本机所有外网流量全部经过 WireGuard 隧道；一旦隧道故障会造成整机断网，同时客户端可借助隧道路由访问服务端全网资源，存在内网越权访问风险。

4. WireGuard 的公私钥模型和 SSH 很像。假设攻击者只拿到了 vpn 节点的私钥，但没有 client 节点的私钥，他能以 client 的身份成功建立隧道吗？为什么？

   > 答：不能。WireGuard 双向身份校验：客户端用自身私钥签名、服务端用客户端公钥验签；攻击者仅有服务端私钥，无法伪造客户端签名，不能冒充客户端建立隧道。

5. 底层抓包看到的外层源地址是什么？为什么抓包者看不到内层的真实通信内容？

   > 答：外层源地址：192.0.2.2；内层报文被 WireGuard 高强度加密封装在 UDP 载荷内，底层抓包只能获取密文，无法解析原始明文与真实内网地址。

6. 任务四中内外两层抓包看到的信息完全不同，这说明 VPN 的"封装"机制给通信提供了哪两方面的安全保障？

   > 答：①内容加密：内层通信数据全加密，链路窃听无法获取业务明文；
②地址隐藏：外层只暴露公网 IP，隐藏内网真实网段与主机 IP，隐蔽内网拓扑。
