...
| 项目                                   | 你的填写 |
| -------------------------------------- | -------- |
| 是否看到 `latest handshake`            |       是（显示具体时间，代表握手成功）   |
| 是否看到 `transfer` 计数               |   是（收发数据包字节数不为 0）       |
...
| 是否能访问 `10.20.0.2:8080`  |  可以正常访问，返回网页目录列表（curl 输出 html 页面）        |
| `ip route` 中 wg0 相关的条目 |10.10.0.0/24 dev wg0 proto kernel scope link src 10.10.10.1 10.20.0.0/24 dev wg0 scope link          |
...
| `latest handshake` 是否还在      | 是，握手状态正常存在         |
| 去 `10.20.0.0/24` 的路由是否消失 |  是，该网段路由被移除，新增10.30.0.0/24 dev wg0路由        |
| 是否还能访问 `10.20.0.2:8080`    |  不能，curl 连接超时失败        |
| 失败的直接原因                   |      系统无10.20.0.0/24指向 wg0 的路由，访问该地址的报文从默认网关转发，不走 WireGuard 隧道    |

> 简答：为什么 `wg show` 显示隧道还在，但业务访问已经失败？隧道握手和业务数据传输用的是两套不同的路径还是同一套？答：wg show 的握手基于底层 UDP 的公网 underlay 链路（192.0.2.x），不受 AllowedIPs 影响；业务转发依赖内核路由表，AllowedIPs 变更后 wg-quick 删除了去往10.20.0.0/24的隧道路由，因此业务不通。
握手和业务数据是两套路径：握手走 underlay 底层 UDP 路由，业务流量受 AllowedIPs 生成的内核路由控制。
...
| ------------------------ | ---------------------------------- | -------- |
| 看到的源地址             |   192.0.2.2（client 底层公网 IP）                                 | 10.10.10.1（客户端隧道内网 IP）         |
| 看到的目的地址           |    192.0.2.1（vpn 底层公网 IP）                                | 网 IP）10.20.0.2（后端业务内         |
| 协议                     | UDP（WireGuard 封装外层传输协议）                                   |  ICMP/HTTP（原始业务明文协议）       |
| 是否能看到 ICMP/HTTP 内容 |       不能                             |   可以       |
| 为什么内外两层看到的不同 | WireGuard 做了UDP 加密封装：原始内网报文被整体加密后装进 UDP 报文，underlay 传输加密后的外层 UDP 包                        |  wg0 是解封装后的原始内网报文              |

> 简答：如果把 VPN 隧道比喻成"信封+信纸"，outer（underlay）抓到的信息对应什么？inner（wg0）抓到的信息对应什么？这样设计对安全性意味着什么？答：outer(underlay)：对应信封，只有两端公网 IP，看不到信封里的真实内容；
inner(wg0)：对应信纸，承载真实内网源目 IP、ICMP/HTTP 原始业务数据；
安全意义：公网链路上只能捕获加密 UDP 密文，第三方无法窃取内网地址与业务明文数据，实现传输加密与内网信息隐藏，提升链路安全性。
...
1. WireGuard 的 `AllowedIPs` 做了哪两件不同的事？为什么它"看起来像访问控制，实际上首先影响的是路由"？
   ...
   > 答：两件事：①自动生成内核路由，指定网段流量走 wg0 隧道；②报文准入校验，仅源 IP 匹配网段才接收对等端报文。
原因：wg-quick 优先根据 AllowedIPs 生成路由，路由决定流量转发路径，因此先影响路由，再附带访问控制效果。

2. 本实验中，`wg show` 显示隧道在线，但 `curl` 失败。为什么隧道握手和业务数据传输可以互相独立？

   > 答：握手基于底层 UDP 公网链路，不受内核路由、AllowedIPs 管控；业务流量依赖 AllowedIPs 生成的内核隧道路由，路由失效即业务不通，二者路径分离、互相独立。

3. 如果只建立隧道而不限制 AllowedIPs（写成 `0.0.0.0/0`），会带来什么风险？

   > 答：0.0.0.0/0会把本机全部流量强制导入 VPN 隧道，风险：①客户端所有上网流量全走对方节点，隐私泄露；②VPN 节点被攻陷后，可劫持全部流量、窃取数据。

4. WireGuard 的公私钥模型和 SSH 很像。假设攻击者只拿到了 vpn 节点的私钥，但没有 client 节点的私钥，他能以 client 的身份成功建立隧道吗？为什么？

   > 答：不能。WireGuard 双向鉴权，建立隧道需要客户端私钥完成身份签名，仅有服务端私钥无法伪造客户端身份。

5. 底层抓包看到的外层源地址是什么？为什么抓包者看不到内层的真实通信内容？

   > 答：外层源：客户端 underlay 底层 IP（实验192.0.2.2）；
原因：内层报文被 WireGuard 高强度加密，公网抓包只能获取密文，无法解析明文内容。

6. 任务四中内外两层抓包看到的信息完全不同，这说明 VPN 的"封装"机制给通信提供了哪两方面的安全保障？

   > 答：①数据加密：公网链路密文传输，防窃听；②地址隐藏：外层仅暴露底层公网 IP，隐藏内网真实网段与业务地址。
