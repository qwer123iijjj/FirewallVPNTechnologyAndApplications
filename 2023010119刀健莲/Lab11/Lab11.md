...
| 项目                                   | 你的填写 |
| -------------------------------------- | -------- |
| 是否看到 `latest handshake`            |是，client 端最新握手为 1 分钟 13 秒前，vpn 端最新握手为 1 分钟 20 秒前|
| 是否看到 `transfer` 计数               |是，client 端：接收 92B、发送 540B；vpn 端：接收 276B、发送 92B|
...
| 项目                         | 你的填写 |
| ---------------------------- | -------- |
| `AllowedIPs` 的值            |10.10.10.0/24, 10.20.0.0/24|
| 是否能访问 `10.20.0.2:8080`  |能|
| `ip route` 中 wg0 相关的条目 |1. 10.10.10.0/24 dev wg0 proto kernel scope link src 10.10.10.1  2. 10.20.0.0/24 dev wg0 scope link|
...
| `latest handshake` 是否还在      |存在（有最新握手记录）|
| 去 `10.20.0.0/24` 的路由是否消失 |路由条目消失|
| 是否还能访问 `10.20.0.2:8080`    |不能（连接失败）|
| 失败的直接原因                   | Peer 的 AllowedIPs 改成10.30.0.0/24后，WireGuard 自动删除10.20.0.0/24指向 wg0 的路由，访问流量无法进入隧道转发|

> 简答：为什么 `wg show` 显示隧道还在，但业务访问已经失败？隧道握手和业务数据传输用的是两套不同的路径还是同一套？
答：握手基于底层 UDP 报文，依靠物理网卡的公网路由，不受 Peer 的 AllowedIPs 配置影响，因此握手正常、隧道状态保留；业务内网10.20.0.0/24流量依赖 AllowedIPs 自动生成的 wg 隧道路由，AllowedIPs 修改后对应路由被删除，业务流量无法送入隧道导致访问失败；握手与业务数据是两套路径。
...
| 协议                     |UDP（端口 56226<->51820）|ICMP|
| 是否能看到 ICMP/HTTP 内容 |不能，载荷是 WireGuard 加密密文|	可以，明文 ICMP/HTTP 原始数据|
| 为什么内外两层看到的不同 |underlay 是外层底层网络，WireGuard 把内网原始报文整体加密封装成 UDP 报文传输；wg0 是隧道内层接口，内核完成解封装，剥离外层 UDP 头部，还原出原始内网 ICMP/HTTP 明文报文，因此两层源目 IP、协议、载荷内容全部不同|          |

> 简答：如果把 VPN 隧道比喻成"信封+信纸"，outer（underlay）抓到的信息对应什么？inner（wg0）抓到的信息对应什么？这样设计对安全性意味着什么？
答：outer (underlay) 抓包对应外层信封，只能看见信封收发地址（公网 IP+UDP 端口）、UDP 协议，信封内部被加密看不到信纸；inner (wg0) 抓包对应里面的信纸，是解密后的原始内网 IP、ICMP/HTTP 业务明文。安全意义：底层链路只能截获加密信封，无法窃取内网真实业务数据与内网网段信息，实现内网报文全加密传输，提升通信保密性。
...
1. WireGuard 的 `AllowedIPs` 做了哪两件不同的事？为什么它"看起来像访问控制，实际上首先影响的是路由"？
   ...
   > 答：AllowedIPs 一是自动生成隧道路由，把指定网段路由指向 wg0 接口，匹配流量送入隧道加密转发；二是做入站访问控制，只放行源 IP 落在 AllowedIPs 网段的对端报文。它优先生成内核路由，路由直接决定流量走不走隧道，路由消失就无法连通，因此看似 ACL，本质先管控路由转发。

2. 本实验中，`wg show` 显示隧道在线，但 `curl` 失败。为什么隧道握手和业务数据传输可以互相独立？

   > 答：握手依靠底层 UDP 公网路由，不受 AllowedIPs 与 wg 隧道路由约束，只要两端公网 IP 互通就能完成密钥协商；业务流量依赖 AllowedIPs 自动生成的 wg 内网路由，路由被删除后业务报文无法进入隧道，因此握手正常、业务不通，二者路径相互独立。

3. 如果只建立隧道而不限制 AllowedIPs（写成 `0.0.0.0/0`），会带来什么风险？

   > 答：AllowedIPs=0.0.0.0/0会自动生成默认路由，本机所有外网流量全部涌入 VPN 隧道；一是本地上网流量全部被引流至对端服务器，网络异常、上网瘫痪；二是对端一旦恶意配置路由，可劫持全部流量窃取数据；三是收到任意来源 IP 的入站数据包都会被接收，丧失访问控制，暴露内网带来入侵风险。

4. WireGuard 的公私钥模型和 SSH 很像。假设攻击者只拿到了 vpn 节点的私钥，但没有 client 节点的私钥，他能以 client 的身份成功建立隧道吗？为什么？

   > 答：不能；WireGuard 是双向公钥校验，节点私钥仅用于证明自身身份，接入客户端时需要校验客户端公钥，攻击者无客户端私钥就无法生成合法签名通过对端公钥验签，没法冒充客户端完成握手建隧道。

5. 底层抓包看到的外层源地址是什么？为什么抓包者看不到内层的真实通信内容？

   > 答：底层 underlay 抓包外层源地址是客户端物理网口公网 IP192.0.2.2；因为 WireGuard 把内层原始报文整体加密后封装进 UDP 数据包，密文存放在 UDP 载荷里，抓包只能拿到加密后的外层 UDP 包，没有密钥无法解密，所以看不到内层 ICMP、HTTP 明文内容。

6. 任务四中内外两层抓包看到的信息完全不同，这说明 VPN 的"封装"机制给通信提供了哪两方面的安全保障？

   > 答：一是地址隐藏，外层只暴露底层公网 IP，内网私网地址被封装隐藏，无法通过抓包获知内网拓扑；二是载荷加密，内层业务数据全密文封装，中间人抓包不能解析 ICMP、HTTP 明文内容，保障数据保密性。
