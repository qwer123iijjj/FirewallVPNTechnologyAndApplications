...
| 是否看到 `latest handshake` |是，显示最新握手时间 |
...
| `client -> server:2222` | 成功 | 失败|
| `client -> server ping` | 成功 |失败 |
...
| :--- | :--- | :--- | :--- |
| `client -> server:8080` | 成功| 行 1、行 2|是 |
| `client -> server:2222` |失败 |行 3、行 4	 | 是|
| `client -> server ping` |失败 | 行 3、行 4	| 是|
...
| 访问目标 | 经过的链 | 原因 |
| :--- | :--- | :--- |
| `client -> server:8080` | `FORWARD` |流量经过 VPN 网关转发至内网服务器，目的不是网关本机 |
| `client -> server:2222` | `FORWARD` | 流量经过 VPN 网关转发至内网服务器，目的不是网关本机|
| `client -> vpn:9090` | `INPUT` |流量目的为 VPN 网关本机进程，不做转发 |
...
| 故障 | 现象 | 原因 | 修复方法 |
| :--- | :--- | :--- | :--- |
| 删除 `ESTABLISHED,RELATED` |访问 8080 超时 / 失败 |客户端请求放行，但服务器回包无规则匹配，被默认 DROP | 在 FORWARD 首行重新添加 ESTABLISHED,RELATED 放行规则|
| `LOG` 放在 `REJECT` 后面 |访问被拒绝，但无对应日志 |iptables 匹配到 REJECT 后终止流程，后续 LOG 规则不会执行 |调整规则顺序，LOG 在前，REJECT 在后 |
| 管理面规则错写到 `FORWARD` |依然可以访问 9090 端口，无日志、计数器不上涨 |访问网关本机走 INPUT 链，FORWARD 链规则对其无效 |删除 FORWARD 中错误规则，将拦截 / 日志规则写入 INPUT 链 |
...
| 测试 | 结果 | 原因 |
| :--- | :--- | :--- |
| `client -> server:8080` | 成功|iptables 规则明确放行该流量 |
| `client -> server:2222` | 失败|未配置放行规则，被 REJECT 拒绝 |
| `client -> server ping` |失败 |	未配置 ICMP 放行规则，被 REJECT 拒绝 |

### D. 日志与抓包

| 项目 | 你的填写 |
| :--- | :--- |
| `VPN-DENY` 日志条数 |多条（根据测试次数变化） |
| 日志中被拒绝的目标端口 | 2222|
...
AllowedIPs：路由转发规则，仅决定哪些流量进入 WireGuard 加密隧道，控制流量走向，不区分端口、协议。
iptables：访问控制策略，对进入系统的流量做协议、端口、源目地址精细化放行 / 拒绝，实现最小权限。
原因：AllowedIPs 只能控制网段，无法限制端口与服务，VPN 用户会获得整个网段所有服务的访问权限，存在安全风险，因此必须配合防火墙做细粒度管控。
2. 为什么 VPN 用户访问内网服务器时，规则要写在 `FORWARD` 链，而不是 `INPUT` 链？
客户端流量经过 VPN 网关转发到内网服务器，流量目的地不是 VPN 网关本机，Linux 网络栈会走FORWARD转发链；INPUT链仅处理目的为本机进程的流量，因此转发流量需配置在FORWARD链。
3. 本实验中 `ESTABLISHED,RELATED` 规则如果删除，访问 `server:8080` 会出现什么现象？
客户端的请求包可以正常放行，但内网服务器返回的应答包属于已建立连接的回包，无对应规则匹配，会被FORWARD默认策略 DROP，最终表现为连接超时、访问失败。
4. 为什么 LOG 必须放在 REJECT 前面？
iptables 规则从上到下依次匹配，命中REJECT后会直接终止当前链的匹配流程。若 LOG 在后，流量被拒绝后不会执行日志规则，无法完成审计记录；因此日志规则必须在拒绝规则之前。
5. underlay 抓包只能看到 UDP 51820，这对通信安全意味着什么？它还能暴露哪些信息？
安全意义：内层 IP、端口、业务数据全程加密，底层网络抓包无法窃取明文业务流量，通信保密性强。
可暴露信息：两端公网 IP、固定 UDP 端口 51820、隧道流量大小、通信时长，攻击者可识别 WireGuard 服务并进行端口扫描、流量分析。
6. 如果一个真实企业允许 VPN 用户访问整个内网 `10.0.0.0/8`，可能带来哪些风险？
VPN 用户可访问内网所有服务器、数据库、运维管理端口，横向渗透风险极高；
内网高危服务（远程桌面、数据库、运维端口）直接暴露在外网接入端；
一旦 VPN 账号泄露，攻击者可完全入侵内网；
无法做细粒度审计，非法访问难以追溯。
7. `client -> vpn:9090` 和 `client -> server:8080` 分别经过哪条 iptables 链？为什么不同？
client -> vpn:9090：走 INPUT 链，流量目的是 VPN 网关本机进程。
client -> server:8080：走 FORWARD 链，流量仅经过网关转发，目的地为内网其他主机。
核心区别：流量最终目的地是否为网关本机。

8. conntrack 能看到 WireGuard 外层 UDP 包和内层 TCP 连接。为什么不能把 WireGuard 隧道本身理解成 conntrack 的 NAT 映射？
WireGuard 是二层 / 三层加密隧道，对整包加密封装，不是地址转换；NAT 只修改 IP / 端口，不加密数据。
隧道状态由 wg show 独立维护，conntrack 仅跟踪解密后的内层业务连接和外层 UDP 通信，不管理隧道握手、密钥、保活等隧道状态。
9. 如果 VPN 网关上有 SSH 管理端口，应该允许哪些来源访问？应该如何写日志？
允许来源：仅企业内网管理网段、固定运维 IP，禁止 VPN 客户端、外网任意地址访问。
日志配置：在INPUT链针对 SSH 端口添加LOG规则，打上专属日志前缀，限制日志频率，记录源 IP、时间，方便审计溯源
10. 当 `wg show` 有握手但业务访问失败时，应该按什么顺序检查服务监听、路由表、防火墙规则、日志和计数器？
排查顺序（由近到远、由基础到策略）：
查看 wg show 确认隧道握手、AllowedIPs 正常；
检查两端路由表，确认业务网段路由指向 wg0 接口；
检查服务监听状态，确认内网服务正常监听端口；
查看 iptables 规则 + 计数器，判断流量是否被防火墙拦截；
查看 系统日志，根据拒绝日志定位拦截原因。
---
