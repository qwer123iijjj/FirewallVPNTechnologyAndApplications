...
| :--- | :------------------ | :--------------------- |
| `server.py` 已启动，`client.py` 尚未运行 |LISTEN |阶段 1：创建监听套接字，等待连接 |
| `client.py` 已连接，数据尚未发送完毕 |ESTABLISHED |阶段 3：数据传输中 |
| `server.py` 执行 `close()` 之后 | TIME_WAIT（服务端）/ CLOSED|阶段 4：连接释放完成 |
...
| 是你调用 `close()` 触发的断开，还是服务器主动断开的 |服务器主动断开（响应头含 Connection: close） |
...
| :--- | :------- | :------- |
| 程序是否自动退出 |是 |是（服务器返回了 Connection: close，未保持长连接） |
| `ss` 中连接的 State |TIME_WAIT / CLOSED |正常退出后为 TIME_WAIT；若服务器启用 Keep-Alive 且无 Connection: close，则会卡在 ESTABLISHED |
| 需要手动 Ctrl+C 才能结束吗 |不需要 |不需要（服务器主动关闭了连接） |
...
1. 实验中 `server.py` 必须比 `client.py` 先启动。如果顺序反过来，`client.py` 会报什么错误？用今天学到的概念解释：此时套接字存在吗？管道存在吗？
   ...
   > 答：会抛出 Connection refused 异常。此时客户端套接字已创建，但服务端未启动，没有处于 LISTEN 状态的套接字，因此 TCP 三次握手无法完成，不存在任何连接管道。

2. `server.py` 打印出了两个不同的描述符编号（`server` 和 `conn`）。为什么 `accept()` 要返回一个新的套接字，而不是直接复用原来的 `server` 套接字？

   > 答：server 套接字（832）是监听套接字，只负责接收新连接请求；conn 套接字（888）是通信套接字，专门与单个客户端传输数据。分离职责可以让服务器同时处理多个客户端，避免监听和传输逻辑冲突。

3. 描述符和端口号都可以用来标识一个套接字，它们的本质区别是什么？各自解决了什么问题？

   > 答：端口号：用于网络寻址，标识主机上的服务（如 8080），解决 “网络上找谁” 的问题。文件描述符：操作系统给进程分配的句柄编号（如 832/888），解决 “进程内如何操作该套接字” 的问题。

4. `client.py` 中，你有没有指定过客户端自己的端口号？这个端口号是谁分配的？在 `ss` 的输出中能看到它吗？

   > 答：未指定客户端端口。端口由操作系统自动分配临时端口（如截图中的 56746），可以在 ss 命令中看到该端口号。

5. HTTP/1.0 实验中，即使你没有调用 `close()`，`recv()` 循环也会自动退出。解释这是为什么？

   > 答：HTTP/1.0 默认使用短连接，服务器发送完响应后会主动发送 FIN 包关闭连接，此时 recv() 会返回空字节，循环条件不成立，因此自动退出。

6. HTTP/1.1 实验中，程序卡在 `recv()` 不退出，`ss` 显示连接仍是 `ESTABLISHED`。这说明 HTTP/1.1 和 HTTP/1.0 在连接管理上有什么根本区别？

   > 答：HTTP/1.1 默认开启持久连接（Keep-Alive），服务器不会在发送完响应后主动关闭连接。如果服务器未返回 Connection: close，客户端会一直等待后续数据，导致 recv() 阻塞，程序无法退出。
