...
| `ip route` 中 wg0 相关的条目 | 10.10.1.0/24 dev wg0 proto kernel scope link src 10.10.1.1;10.20.0.0/24 dev wg0 scope link         |
...
| 去 `10.20.0.0/24` 的路由是否消失 |已消失，替换为 10.30.0.0/24 dev wg0 路由          |
| 是否还能访问 `10.20.0.2:8080`    |不能，curl 返回连接失败          |
| 失败的直接原因                   | 10.20.0.0/24没有指向 wg0 的路由，访问该网段的数据包无法送入 WireGuard 隧道转发         |

> 简答：为什么 `wg show` 显示隧道还在，但业务访问已经失败？隧道握手和业务数据传输用的是两套不同的路径还是同一套？
隧道握手：依靠外层 underlay 地址192.0.2.1、Endpoint地址通信，不受 AllowedIPs 参数影响，所以握手还存在；
业务数据：由 AllowedIPs 生成对应网段路由，只有目标 IP 匹配 AllowedIPs 网段才会进 wg0 隧道；当前 AllowedIPs 改为10.30.0.0/24，10.20.0.2无对应路由，报文无法进入隧道，访问失败。
截图：
...
| ------------------------ | ---------------------------------- | -------- |
| 看到的源地址             |  	192.0.2.2                                  |10.10.1.1          |
| 看到的目的地址           |  192.0.2.1                                  | 	10.20.0.2         |
| 协议                     | UDP                                   | 	ICMP         |
| 是否能看到 ICMP/HTTP 内容 |  不能（数据加密）                                  |  可以（明文 ICMP，截图可见 echo request/reply）        |
| 为什么内外两层看到的不同 | underlay 是 WireGuard 外层 UDP 加密封装，内层整个 IP 包被加密；wg0 是解密后的原始内层报文，地址、协议都是真实业务报文                                   |underlay 是 WireGuard 外层 UDP 加密封装，内层整个 IP 包被加密；wg0 是解密后的原始内层报文，地址、协议都是真实业务报文          |

> 简答：如果把 VPN 隧道比喻成"信封+信纸"，outer（underlay）抓到的信息对应什么？inner（wg0）抓到的信息对应什么？这样设计对安全性意味着什么？
outer (underlay) 对应信封：仅能看到外层公网 IP 地址（192.0.2.x），内部报文被加密看不到 ICMP 内容；
inner (wg0) 对应信纸：拆封解密后的原始明文数据包，可看到内网 IP 和 ICMP 载荷；
安全性：链路传输密文，中间人抓外层无法窃取内网地址与业务数据。
截图：
...
1. WireGuard 的 `AllowedIPs` 做了哪两件不同的事？为什么它"看起来像访问控制，实际上首先影响的是路由"？
   ...
   > 答：① 路由下发：wg-quick 依据AllowedIPs自动在本机添加路由，目标网段流量转发至 wg0 网卡；
② 入站白名单校验：对端发来的内层 IP 包，源 IP 不在本端 AllowedIPs 网段则直接丢弃，实现访问控制。
原因：数据包优先遵循内核路由表转发，若没有对应隧道路由，流量根本无法进入 WireGuard 隧道，因此路由是前置生效逻辑，访问控制是报文到达隧道后的后置校验，所以优先影响路由。

2. 本实验中，`wg show` 显示隧道在线，但 `curl` 失败。为什么隧道握手和业务数据传输可以互相独立？

   > 答：握手和业务流量依托两套独立通路：
隧道握手：使用外层 underlay 的 UDP（Endpoint 公网 IP + 端口）通信，只校验公私钥，和内层路由、AllowedIPs 无关，只要底层网络可达就能完成握手；
业务传输：依赖 AllowedIPs 生成的内层路由，路由缺失 / 网段错误时，业务报文无法送入 wg 隧道，因此出现隧道在线、curl 访问失败。

3. 如果只建立隧道而不限制 AllowedIPs（写成 `0.0.0.0/0`），会带来什么风险？

   > 答：AllowedIPs=0.0.0.0/0代表所有 IP 流量全走隧道，风险：
① 路由泛滥：本机所有外网流量全部经 VPN 转发，造成带宽损耗；
② 内网横向风险：VPN 对端一旦被攻陷，可通过隧道路由访问本机全量网段；
③ 安全泄露：本机任意流量都通过隧道加密转发，违背最小权限原则。

4. WireGuard 的公私钥模型和 SSH 很像。假设攻击者只拿到了 vpn 节点的私钥，但没有 client 节点的私钥，他能以 client 的身份成功建立隧道吗？为什么？

   > 答：不能。WireGuard 是双向非对称密钥认证：建立隧道时，客户端需要用自身私钥签名，服务端用客户端公钥验签；攻击者仅有 vpn 私钥，没有 client 私钥，无法伪造客户端身份完成签名，不能冒充 client 接入隧道。

5. 底层抓包看到的外层源地址是什么？为什么抓包者看不到内层的真实通信内容？

   > 答：外层源地址：192.0.2.2（client 的 underlay 底层 veth 地址）；
看不到内层内容原因：WireGuard 对整个内层 IP 报文做对称加密，底层链路只传输加密后的 UDP 载荷，抓包只能拿到密文，无法解析明文 ICMP / 业务 IP 信息。

6. 任务四中内外两层抓包看到的信息完全不同，这说明 VPN 的"封装"机制给通信提供了哪两方面的安全保障？

   > 答：地址隐藏：外层仅暴露 underlay 公网 IP，隐藏了内网真实业务 IP（10.10.x/10.20.x），规避内网网段泄露；
载荷加密：内层原始报文全密文封装在 UDP 中，链路窃听者无法解析通信内容，防止数据窃取。
