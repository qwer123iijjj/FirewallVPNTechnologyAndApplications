...
| 网关 | 允许规则 | 日志前缀 | 默认策略 |
| :--- | :--- | :--- | :--- |
| `gwA` |1. 所有已建立 / 相关连接的流量（ctstate RELATED,ESTABLISHED）2. 来自 10.1.0.0/24 网段、去往 10.2.0.2:8080 的 TCP 新建连接（ctstate NEW）|A-TO-B-DENY:|DROP|
| `gwB` |1. 所有已建立 / 相关连接的流量（ctstate RELATED,ESTABLISHED）2. 来自 10.1.0.0/24 网段、去往 10.2.0.2:8080 的 TCP 新建连接（ctstate NEW）|VPN-S2S-DENY:|DROP|
...
| 测试 | 预期结果 | 实际结果 | 日志 |
| :--- | :--- | :--- | :--- |
| `siteA -> siteB:8080` | 成功 |成功|无拦截日志（流量被防火墙放行，未触发 LOG 拒绝规则）|
| `siteA -> siteB:2222` | 失败 |失败|存在A-TO-B-DENY前缀的 TCP 2222 端口拦截日志，无VPN-S2S-DENY日志|
| `siteA -> siteB ping` | 失败 |失败|存在多条A-TO-B-DENY前缀的 ICMP 类型 8 ping 拦截日志，无VPN-S2S-DENY日志|
...
| `gwB` LAN 地址 |10.2.0.1|
| `siteB` 地址 |10.2.0.2|
| underlay 是否连通 |连通（gwA 与 gwB 公网地址互相 ping 测试 0% 丢包，底层 underlay 网络正常互通）|
...
| `latest handshake` |16 seconds ago|16 seconds ago|
| `transfer` 计数 |124 B received, 180 B sent|180 B received, 272 B sent|
...
| 测试 | 结果 | 命中的规则 |
| :--- | :--- | :--- |
| `siteA -> siteB:8080` |连通成功|1. 第 1 条规则：允许RELATED,ESTABLISHED状态流量2. 第 2 条规则：放行源网段10.1.0.0/24去往10.2.0.2的 TCP 8080 新建连接|
| `siteA -> siteB:2222` |访问失败（连接被拒绝）|1. 先匹配前两条放行规则均不满足2. 命中第 3 条LOG日志记录规则（日志前缀A-TO-B-DENY）3. 命中第 4 条REJECT拒绝规则，返回 ICMP 端口不可达|
| `siteA -> siteB ping` |连通失败（100% 丢包）|1. 前两条放行规则不匹配（仅放行 TCP 8080 流量）2. 命中第 3 条LOG规则，记录 ICMP 拒绝流量日志3. 命中第 4 条REJECT规则丢弃数据包|

### E. 抓包对比

| 抓包位置 | 看到的地址 | 协议 | 说明 |
| :--- | :--- | :--- | :--- |
| underlay |源：192.0.2.1，目的：192.0.2.2|UDP（目的端口 51820，WireGuard 默认端口）|WireGuard 外层封装报文，属于底层公网隧道传输，原始内网 IP 被加密封装，只能看到两端公网 underlay 地址，无法识别内网业务报文内容|
| `wg0` |源：10.1.0.2，目的：10.2.0.2|TCP + HTTP|隧道解封装后的原始内网业务报文，可以看到站点 A、B 的内网私有 IP，完整的 TCP 三次握手与 HTTP 访问流量，WireGuard 在此处完成数据包的加解密|
| B 站点 LAN |源：10.1.0.2，目的：10.2.0.2|TCP + HTTP|流量从网关转发到站点 B 内网，为原始未加密的内网业务数据包，目的主机直接接收该 HTTP 请求，和 wg0 内的原始业务报文一致|
...
>答：组网场景不同
远程接入 VPN：面向移动个人用户，员工在外通过电脑 / 手机单点接入企业内网；
站点到站点 VPN：面向两个固定局域网，实现总部与分支机构整个网段永久互通。
拓扑结构不同
远程接入：一对多，单个用户网关对接多个远端客户端；
站点到站点：网关对网关，两端 VPN 设备固定建立隧道。
地址分配方式不同
远程接入：客户端接入后由 VPN 网关动态分配虚拟内网 IP；
站点到站点：两端内网网段提前固定规划，通过路由转发实现全网段互通。
使用时效不同
远程接入：按需临时拨号连接，下线后隧道断开；
站点到站点：永久在线，网关间持续维持隧道。
2. 为什么站点到站点 VPN 中 `AllowedIPs` 应该写对端内网网段？
>答：AllowedIPs 两个核心作用：
路由自动下发：WireGuard 会自动为配置的网段生成指向wg0接口的路由，去往该网段的流量全部送入 VPN 隧道封装转发；
访问权限控制：只有源 / 目的 IP 匹配该网段的数据包，才允许通过隧道收发，过滤非法流量，防止 VPN 隧道被滥用。
如果只填写对端隧道接口 IP，只能实现网关之间互通，无法让两端内网主机互相访问。
3. 为什么两个网关都必须开启 `net.ipv4.ip_forward=1`？
>答：VPN 网关需要充当三层路由器，接收本局域网主机流量，转发到 WireGuard 隧道接口wg0；同时把对端解密后的内网流量转发到本地 LAN 网段。
ip_forward=0时，Linux 系统默认丢弃跨网卡转发的数据包，内网跨网段流量无法在veth-lan和wg0之间转发，两端主机无法互通。
双向流量都需要转发，因此两端网关必须同时开启 IP 转发。
4. 为什么两个站点内的普通主机都需要把默认网关指向本地 VPN 网关？
>答：普通主机没有去往对端内网的静态路由，访问跨站点网段时，数据包会发送给本机配置的默认网关（本地 VPN 网关）。
VPN 网关预先存在对端内网的隧道路由，收到主机数据包后，转发进 WireGuard 隧道完成跨站点传输。
若主机网关不是 VPN 网关，跨网段流量会被发送到其他路由设备，无法进入 VPN 隧道，站点间无法通信。
5. `wg show` 里有握手但业务不通，排查顺序应该是什么？
>答：检查两端网关net.ipv4.ip_forward是否开启；
核对两端AllowedIPs配置是否包含完整对端内网网段，确认自动路由是否生成且出接口为wg0；
检查两端防火墙（iptables）FORWARD 链是否放行 LAN 与 wg0 之间的转发流量；
检查两端主机默认网关是否正确指向本地 VPN 网关；
在网关、内网主机分别抓包，确认数据包是否进入 wg0 隧道、是否被防火墙拦截丢弃；
核对 WireGuard 对端公网 Endpoint 地址、端口、公私钥是否配置无误。

6. 本实验中 `gwA` 和 `gwB` 都写防火墙规则，有什么好处？只在一边写规则可以吗？
>答：双向安全防护：既管控 A 站点主动访问 B 站点的流量，也限制 B 站点主动访问 A 站点的流量，实现双向访问权限管控；
故障域隔离：流量在发起端网关即可拦截，不需要经过 VPN 隧道传输再拒绝，节省带宽，同时避免恶意流量进入内网；
精细化双向策略审计：两端可以分别记录各自方向的拒绝流量日志，方便安全审计与故障排查。
只在一边写规则不可以
仅单侧防护只能限制单向访问，反向流量不受约束，会造成安全策略漏洞；
被拒绝流量需要完整经过 VPN 隧道传输后才被丢弃，浪费隧道带宽，恶意流量会进入内网区域，带来安全风险；
只能采集单侧访问日志，无法完整审计双向流量行为。

7. underlay 抓包看不到 `10.1.0.2` 和 `10.2.0.2`，这说明 VPN 隐藏了哪些信息？还暴露了哪些信息？
>答：被隐藏的信息
两端内网私有 IP 地址、源目的内网主机身份；
上层业务协议（TCP/HTTP 等）、访问的端口、请求内容、传输的业务数据；
内网的网络拓扑、开放的业务服务。
暴露的信息
两端 VPN 网关的 underlay 公网 IP 地址；
WireGuard 使用的 UDP 传输协议与监听端口（默认 51820）；
两端之间存在持续的 UDP 流量，可以识别出此处部署了 VPN 隧道。
8. 如果要允许 `siteB` 主动访问 `siteA` 的某个服务，需要增加哪些规则？
>答：gwB 防火墙规则：在 gwB 的 FORWARD 链放行从wg0去往 siteA 内网对应服务端口的新建 TCP 流量；
gwA 防火墙规则：在 gwA 的 FORWARD 链放行从wg0进入、访问本地该服务端口的流量；
确保两端防火墙RELATED,ESTABLISHED状态的回程连接放行规则已存在（本次实验已有该规则，无需重复添加）；
确认两端AllowedIPs配置已包含对端完整内网网段，保证回程路由可达。
